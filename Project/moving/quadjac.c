...
double rprimealph(double *x, double t);
double getvel(double h, int i, int p, double *x);
/* LU factorization */
...
void main(int nargs, char *argv[]) {
  ...
  double h, theta, tmax=1.0, *xLeg, *wLeg, **vtx, xdiff, ydiff, norm, t0, t1, ny[2], fnodal, solnquad[2], *fquad, *fnode, **ftild, *B, *C, **V, **K, solution[8]={0,0,0,0,0,0,0,0}, *innerrhs, *innerdubdn, solnnodal, *z, solndubdn[2], velnodal, *y, val1, val2, yy, maxerr, l2error, s;
...
//        getfquad(h, i, p, &pnls[i][k], &pnls[i][k], xLeg, wLeg, solnquad); //nonmoving geometry
    ...
    for(j = 0; j < nVtx; j++){
      solnnodal = getfnode(h, i, p, vtxall[i][j]);
      fnode[j] = solnnodal;
    }
    f[i] = fnode;  
    ...
    //build velocity
    double *vel;
    vel = (double *)calloc(nVtx, sizeof(double));
     for(j = 0; j < nVtx; j++){
      velnodal = getvel(h, i, p, vtxall[i][j]);
      vel[j] = velnodal; //moving geometry
//      vel[j] = 0.0; //nonmoving geometry
    }
    velall[i] = vel;
    ...
    for(m = 0; m <= i; m++){
      ...
      for(pnlsX = 0; pnlsX < nPnls; pnlsX++){
        for(pnlsY = 0; pnlsY < nPnls; pnlsY++){
          ...
          K[m][pnlsX + nPnls*Yidxv0] += solution[4]+solution[6] - (solution[0]+solution[2])*velall[m][Yidxv0];
          K[m][pnlsX + nPnls*Yidxv1] += solution[5]+solution[7] - (solution[1]+solution[3])*velall[m][Yidxv1];
        }
      }
/*
      printf("----matrix V[%d]------\n",m);
      ...
      for(z = 0; z < nVtx; z++){
        for(j = 0; j < nVtx; j++){
          printf("%f ",V[m][z*nVtx+j]);
        } printf("\n");
      }
      printf("----matrix K[%d]------\n",m);
      for(z = 0; z < nVtx; z++){
        for(j = 0; j < nVtx; j++){
          printf("%.11f ",K[m][z*nVtx+j]);
        } printf("\n");
      }
*/
    }//end m-loop
...
void getfquad(double h, int i, int p, panel *pX0, panel *pX1, double xLeg[], double wLeg[], double solnquad[]){
  ...
  length  = (v10[0] - v00[0])*(v10[0] - v00[0]);
  length += (v10[1] - v00[1])*(v10[1] - v00[1]);
  length = sqrt(length);
